...
export function BackupBar({ entries, onImport }: BackupBarProps) {
  const fileRef = useRef<HTMLInputElement>(null)

  function handleExport() {
    const payload = { app: 'kulturliste', version: 1, entries }
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'kulturliste-backup.json'
  ...
  async function handleFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const list: Entry[] = Array.isArray(parsed) ? parsed : parsed.entries
      if (!Array.isArray(list)) throw new Error('ungültig')
      onImport(list)
    } catch {
      alert('Diese Datei konnte nicht gelesen werden. Bitte wähle eine Kulturliste-Backup-Datei (.json).')
    }
  }
  ...
  return (
    <div className="mt-10 pt-4 border-t border-neutral-200 dark:border-neutral-800 flex items-center justify-center gap-4 text-sm text-neutral-500 dark:text-neutral-400">
      <button type="button" onClick={handleExport} className="underline underline-offset-2" disabled={entries.length === 0}>
        Sichern (Backup)
      </button>
