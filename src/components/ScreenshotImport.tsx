...
export function ScreenshotImport({ onSubmit, onCancel, existingTags = [] }: ScreenshotImportProps) {
  ...
  async function handlePasteFromClipboard() {
    ...
    if (!navigator.clipboard?.read) {
      setError('Einfügen aus der Zwischenablage wird von diesem Browser nicht unterstützt. Bitte Screenshot auswählen.')
      return
    }
    try {
      ...
      setError('Kein Bild in der Zwischenablage gefunden. Screenshot erst kopieren, dann hier einfügen.')
    } catch (err) {
      console.error(err)
      setError('Zugriff auf die Zwischenablage war nicht möglich. Bitte Screenshot stattdessen auswählen.')
    }
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <button
        ...
        className="rounded-md border border-neutral-300 dark:border-neutral-700 py-2.5 text-center text-neutral-700 dark:text-neutral-300 font-medium"
      >
